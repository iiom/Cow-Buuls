class Bulls
  ...
  # совпадение для "cow"
    if uniq_random_number.join.to_i == user_input
      ...
      array_temp = []
      user_input = user_input.to_s.split('').map(&:to_i) #приведение к Int массиву
      user_input.size.times do |i|    # метод times по длине массива ввода игрока
        array_temp << user_input[i] if    #запись во временный массив ввод игрока
        uniq_random_number[i] == user_input[i]        #при условии,что совпадает
                                              # элеменнты по одинаковым индексам
      end
